...
```{r}
beb = read.table('../../data/EnergyDrink', header =  T)
...
```{r}
plot(data, col = c("cyan"), main = "Diagrama de Mosaico")

plot(HairEyeColor, col= c('pink', 'blue'))
...
barplot(t(table(data[1:4])), beside = TRUE,
        legend.text = TRUE,
        col = c("burlywood4", "lightblue", "orange3", "lightgreen"), 
        main = "Frecuencias absolutas del color de ojos\n para cada color de pelo")
```
```{r}
barplot(table(female, male), legend.text = T)
```
```{r}
