...
```{r}
vec = c(16,0,1,20,158,48,5,45,23,11,10,5)
...
```{r}
vec[which(vec>5)]
vec[which(vec>5 | vec%/%2==0)]
vec[which(vec%/%2==0)]

x = c(1,5,2,7,NA, NA, 7,89, NA,9)
...
levels(fac) = c('suspenso', 'aprobado', 'notable', 'excelente')
...
```{r}
apply(A, MARGIN = c(1,2), FUN = function(x){sqrt(sum(x^2))})
...
eigen(M)$vectors %*% diag(eigen(M)$values) %*% solve(eigen(M)$vector)
